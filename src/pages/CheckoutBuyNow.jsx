...
const CheckoutBuyNow = () => {
  ...
  const { cart: existingCart, removeFromCart } = useCart();
  const [cart, setCart] = useState(item ? [item] : []);
  ...
  const applyPromoCode = () => {
    ...
    const userHasCoupon =
      user && user.coupons ? user.coupons.includes(coupon.title) : false;
  ...
  useEffect(() => {
    if (!loading && !isLoggedIn) {
      navigate("/account/login", { item, state: { from: location } });
    }
